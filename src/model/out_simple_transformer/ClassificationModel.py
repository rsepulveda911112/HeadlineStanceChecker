...
from model.out_simple_transformer.classification_utils import (
    InputExample,
...
from model.roberta.RobertaForSequenceClassification import RobertaForSequenceClassification
...
class ClassificationModel():
    def __init__(
        self, model_type, model_name, model_dir=None, num_labels=None, weight=None, args=None, use_cuda=True, cuda_device=-1, **kwargs,
    ):
        ...
        if model_dir:
            self.args = self._load_model_args(model_dir)
        ...
        if num_labels:
            self.config = config_class.from_pretrained(model_load, num_labels=num_labels, **self.args.config)
            self.num_labels = num_labels
        else:
            self.config = config_class.from_pretrained(model_load, **self.args.config)
        ...
        if self.weight:
            self.model = model_class.from_pretrained(
                model_load, config=self.config, weight=torch.Tensor(self.weight).to(self.device), value_head=args['value_head'], **kwargs,
            )
        else:
            self.model = model_class.from_pretrained(model_load, config=self.config, value_head=args['value_head'], **kwargs)
        ...
        if isinstance(train_df, str) and self.args.lazy_loading:
            ...
            if "text" in train_df.columns and "labels" in train_df.columns:
                train_examples = [
                    ...
                    for i, (text, label, feature) in enumerate(zip(train_df["text"].astype(str), train_df["labels"], train_df["features"]))
                ]
                ...
                train_examples = [
                    ...
                    for i, (text_a, text_b, label, feature) in enumerate(
                        zip(train_df["text_a"].astype(str), train_df["text_b"].astype(str), train_df["labels"], train_df["features"])
                    )
                ]
            else:
                warnings.warn(
    ...
    def train(
        ...
        # optimizer = optim.SGD(optimizer_grouped_parameters, lr=args.learning_rate, weight_decay=args.weight_decay)
    ...
    def evaluate(
        ...
        if isinstance(eval_df, str) and self.args.lazy_loading:
            ...
            if "text" in eval_df.columns and "labels" in eval_df.columns:
                eval_examples = [
                    InputExample(i, text, None, label, feature)
                    for i, (text, label, feature) in enumerate(zip(eval_df["text"].astype(str), eval_df["labels"], eval_df["features"]))
                ]
    ...
    def load_and_cache_examples(
        ...
        if output_mode == "classification":
            ...
            all_feature = torch.tensor([f.feature_id for f in features], dtype=torch.float)
    ...
    def predict(self, to_predict, multi_label=False):
        ...
        if multi_label:
            ...
            if len(to_predict[0]) > 2:
                eval_examples = [InputExample(i, text[0], text[1], 0, text[2]) for i, text in enumerate(to_predict)]
            else:
                eval_examples = [InputExample(i, text[0], None, 0, text[1]) for i, text in enumerate(to_predict)]
        if args.sliding_window:
    ...
    def _get_inputs_dict(self, batch):
        if isinstance(batch[0], dict):
            ...
            inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3], "externalFeatures": batch[4]}
